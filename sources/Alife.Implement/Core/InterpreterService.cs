...
public class InterpreterService : InteractivePlugin<InterpreterService>
{
    ...
    public override async Task StartAsync(Kernel kernel, ChatActivity chatActivity)
    {
        ...
        //创建xml解析执行器等
        handlerTable.Register(new XmlHandler(this));
        ...
        sb.AppendLine($"# {nameof(InterpreterService)}");
        sb.AppendLine();
        sb.AppendLine("你可以通过xml格式提供你的文本，xml格式与标准规范完全一致，一些特殊的xml标签还可以充当函数调用，使你的内容发挥特别的效果。");
        sb.AppendLine();
        sb.AppendLine("## 特殊标签");
        sb.AppendLine();
        sb.AppendLine(handlerTable.Document());
        sb.AppendLine();

        if (implicitSummary.Length > 0)
        {
            sb.AppendLine("## 更多工具");
            sb.AppendLine("以下工具目前处于隐式状态，你可以通过 `<help name=\"工具名\" />` 来查看它们的详细使用文档：");
            sb.Append(implicitSummary);
            sb.AppendLine();
        }

        sb.AppendLine("## 注意事项");
        sb.AppendLine();
        sb.AppendLine("1. 注意分清开闭标签和自闭合标签，必须按文档的方式调用。");
        sb.AppendLine("2. 每次回复时，每种开闭标签只能调用一次，所以要把所有内容都放到一个区域中。");
        sb.AppendLine("3. 自闭合标签允许嵌套在开闭标签中，借此可以实现同时执行两种指令。");
        sb.AppendLine("4. 除非是需要调用指令，否则不能再使用xml符号，比如<,>，要用需要使用其他代词或转义。");
        sb.AppendLine();
        sb.AppendLine("## 示例用法");
        sb.AppendLine();
        sb.AppendLine("<say>");
        sb.AppendLine("主人你看我~");
        sb.AppendLine("可以一边跳舞");
        sb.AppendLine("<dance/>");
        sb.AppendLine("一边说话噢。");
        sb.AppendLine("另外我还可以通过 左尖括号python右尖括号 执行脚本呢！");
        sb.AppendLine("</say>");
        sb.AppendLine("<python>");
        sb.AppendLine("print('Hello World!')");
        sb.AppendLine("<python>");
        sb.AppendLine();
        sb.AppendLine("上述xml调用解析：");
        sb.AppendLine("1. 将说话内容放在的say区域中以实现说话输出。");
        sb.AppendLine("2. say期间嵌套‘&lt;dance&gt;’实现了边说话边执行动作。");
        sb.AppendLine("3. 通过‘左尖括号python右尖括号’来避免输出xml符号。");
        sb.AppendLine("4. 在结尾使用python区域进行了脚本调用功能。");

        chatActivity.ChatBot.ChatHistory.AddSystemMessage(sb.ToString());
